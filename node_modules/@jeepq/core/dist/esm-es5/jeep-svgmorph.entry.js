...
var MPoint = /** @class */ (function () {
    ...
    MPoint.prototype.toFixed = function (n) {
    ...
    MPoint.prototype.fromString = function (ptstr) {
        var arr = ptstr.split(',');
        var ret = false;
        if (arr.length === 2) {
            this.x = Number(arr[0]);
    ...
    MPoint.prototype.clone = function () {
        var ret = new MPoint();
        ret.x = this.x;
        ret.y = this.y;
        return ret;
    };
    MPoint.prototype.multNumber = function (a) {
        var ret = new MPoint();
        ret.x = this.x * a;
        ret.y = this.y * a;
        return ret;
    };
    MPoint.prototype.multMPoint = function (coeff) {
        var ret = new MPoint();
        ret.x = this.x * coeff.x;
        ret.y = this.y * coeff.y;
        return ret;
    };
    MPoint.prototype.multMatrix = function (a) {
        var ret = new MPoint;
        ret.x = a[0][0] * this.x + a[0][1] * this.y;
        ret.y = a[1][0] * this.x + a[1][1] * this.y;
        return ret;
    };
    MPoint.prototype.addMPoint = function (pt) {
        var ret = new MPoint();
        ret.x = this.x + pt.x;
        ret.y = this.y + pt.y;
        return ret;
    };
    MPoint.prototype.substractMPoint = function (pt) {
        var ret = new MPoint();
        ret.x = this.x - pt.x;
        ret.y = this.y - pt.y;
        return ret;
    };
    MPoint.prototype.middleMPoint = function (pt) {
        var ret = new MPoint();
        ret.x = (this.x + pt.x) / 2.0;
        ret.y = (this.y + pt.y) / 2.0;
        return ret;
    };
    MPoint.prototype.distance = function (pt) {
        var ret = new MPoint();
        ret.x = pt.x - this.x;
        ret.y = pt.y - this.y;
        return ret;
    };
    MPoint.prototype.dotProduct = function (ptEnd, pt) {
        return (pt.x - this.x) * (ptEnd.x - this.x)
            + (pt.y - this.y) * (ptEnd.y - this.y);
    };
    MPoint.prototype.isMPointOnLine = function (ptEnd, pt) {
        var ret = false;
        var l = this.scalarDistance(ptEnd);
        var r = this.dotProduct(ptEnd, pt) / (l * l);
        var ptP = this.addMPoint(ptEnd.substractMPoint(this).multNumber(r));
        var s = ptP.scalarDistance(pt);
        if (Math.abs(s) < 0.0001)
            ret = true;
        return ret;
    };
    MPoint.prototype.scalarDistance = function (pt) {
        return Math.sqrt((pt.x - this.x) * (pt.x - this.x) + (pt.y - this.y) * (pt.y - this.y));
    };
    MPoint.prototype.atDistanceMPoint = function (pt, distance) {
...
var Matrix = /** @class */ (function () {
    function Matrix(msize) {
        if (msize === void 0) { msize = null; }
        if (msize != null) {
            this._size = msize;
            this.zeros();
        }
    }
    Matrix.prototype.setSize = function (arr) {
        this._size = arr;
        this.zeros();
    };
    ...
    Matrix.prototype.multiplyByVectorMPoints = function (vMPoints) {
        var nMPoints = [];
        var mat = this.matrix;
        if (typeof this._size === "number") {
...
var splitCubicBezier = function (cB, z) {
    ...
    var retCurves = {};
    var r1 = cB[0].isMPointOnLine(cB[3], cB[1]);
    var r2 = cB[0].isMPointOnLine(cB[3], cB[2]);
    if (r1 && r2) {
        retCurves.C1 = [];
        retCurves.C2 = [];
        retCurves.C1 = __spreadArrays(retCurves.C1, [cB[0]]);
        var ptEnd = cB[0].addMPoint(cB[3].substractMPoint(cB[0]).multNumber(z));
        var pt1 = cB[0].addMPoint(ptEnd.substractMPoint(cB[0]).multNumber(1.0 / 3.0));
        var pt2 = cB[0].addMPoint(ptEnd.substractMPoint(cB[0]).multNumber(2.0 / 3.0));
...
var getCubicBezierFromMPoints = function (points) {
    var d = "";
...
var initCurveMPoints = function (P1, P2, P3, P4) {
    var pts = [];
...
var permuteCubicBezier = function (points, startIndex) {
    ...
    var cBz = getCubicBezierFromMPoints(newPoints);
    return cBz;
};
...
var alignSegmentPath = function (cB, ratio) {
    ...
    if (ratio > 1.0) {
        ...
        for (var i = 0; i < points.length - 1; i += 3) {
            ...
            var tmpPt = initCurveMPoints(points[i], points[i + 1], points[i + 2], points[i + 3]);
            while (nbSplit > 1) {
                var z = 1.0 / nbSplit;
                var pts = initCurveMPoints(tmpPt[0], tmpPt[1], tmpPt[2], tmpPt[3]);
...
var lineToCubicBezier = function (cBezier, oPath, letter) {
    var cBz = cBezier;
    var tPoint = new MPoint();
    var nPoint = new MPoint();
    if (letter) {
...
var quadraticToCubicBezier = function (cBezier, oPath, letter) {
    ...
    cBz.cBz = cBz.cBz.concat('C ');
    var cP1 = cBz.lPoint.addMPoint(nP2.substractMPoint(cBz.lPoint).multNumber(2.0 / 3.0));
    cBz.cBz = cBz.cBz.concat(cP1.toFixed(2) + " ");
    var cP2 = nP3.addMPoint(nP2.substractMPoint(nP3).multNumber(2.0 / 3.0));
...
var JeepSvgmorph = /** @class */ (function () {
    ...
    class_1.prototype.render = function () {
        var toRender = [];
        if (this.isSVG) {
            toRender = __spreadArrays(toRender, [h("div", { id: "morph-container" }, h("div", { id: "morph-svg-container" }, h("svg", { id: "morph-svg", width: "500", height: "500", viewBox: "0 0 500 500", xmlns: "http://www.w3.org/2000/svg" }, h("path", { id: "initPath" }, h("animate", { id: "animPath", begin: "initPath.click" }), this._fillColor.length > 1 && !this._uniqueColor
                    ? h("animate", { id: "animFill", begin: "animPath.begin" })
                    : null))))]);
        }
        else {
            toRender = __spreadArrays(toRender, [h("div", { id: "fake-container" })]);
        }
