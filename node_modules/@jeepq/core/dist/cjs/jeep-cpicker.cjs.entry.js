...
const JeepCpicker = class {
    ...
    async parseColorProp(newValue) {
        ...
        this._stateProperties.hsb = colorConverter.hextoHSB(this.innerColor, this.innerOpacity ? this.innerOpacity.toFixed(3) : "1.000");
    ...
    async parseOpacityProp(newValue) {
        ...
        this._stateProperties.hsb = colorConverter.hextoHSB(this.innerColor ? this.innerColor : '#ff0000', this.innerOpacity.toFixed(3));
        if (this._stateProperties.init)
    ...
    async _init() {
        ...
        this._bboxes = {};
        this._oriColor = colorConverter.initSelectedColor(this.innerColor ? this.innerColor : '#ff0000', this.innerOpacity ? this.innerOpacity.toFixed(3) : "1");
    ...
    async _setSelected(color, opacity) {
        if (!this._stateProperties.init) {
            ...
            this._stateProperties.hsb = colorConverter.hextoHSB(color, opacity.toFixed(3));
        }
        ...
        this._stateProperties.color = colorConverter.fillColor(this._stateProperties.hsb);
        ...
        this._stateProperties.hue = (colorConverter.HcolortoHEX(this._stateProperties.hsb)).hex;
        this._stateProperties.colorText = this._setTextColor(this._stateProperties.color.hsb);
        this._stateProperties.colorHandle = this._setTextColor(colorConverter.hextoHSB(this._wrapCss.backColor), 'back');
    ...
    _setTextColor(hsb, t) {
        ...
        let type = t ? t : 'col';
        let opa = colorConverter.opacityfromHSB(hsb);
    ...
    async _pickColor(pt) {
        if (pt.x <= this._bboxes.color.right && pt.x >= this._bboxes.color.left &&
            ...
            return await this._updateStateProperties(this._stateProperties.hsb.h, await this.calcS(pos.x, pos.width), await this.calcB(pos.y, pos.height), colorConverter.opacityfromHSB(this._stateProperties.hsb));
        }
    }
    async _pickHue(pt) {
        if (pt.x <= this._bboxes.hue.right && pt.x >= this._bboxes.hue.left &&
            ...
            return await this._updateStateProperties(await this.calcH(pos.y, pos.height), this._stateProperties.hsb.s, this._stateProperties.hsb.b, colorConverter.opacityfromHSB(this._stateProperties.hsb));
        }
    }
    ...
    async _updateStateProperties(h, s, b, opa) {
        ...
        await this._setSelected(colorConverter.fillColor(color).hex.hex, opa);
        this.toggleDisplay = !this.toggleDisplay;
        return;
    }
    _calcH(y, height) {
