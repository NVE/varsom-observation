...
const JeepLinechart = class {
    constructor(hostRef) {
        ...
        this.toggle = false;
        this._update = false;
        this.readyLinechart = core.createEvent(this, "jeepLinechartReady", 7);
    }
    //*****************************
    ...
    parseDataProp(newValue) {
        ...
        if (status.status === 200) {
            ...
            const retSets = chartSvgelements.checkDataSetsValidity(dataSets, this._axisType);
    ...
    async parseStyleProp(newValue) {
        ...
        if (cstyle != null) {
            const propInstance = await common.getCssPropertyFromString(cstyle);
    ...
    _createTitle() {
        if (this.innerTitle != null) {
            ...
            let scale = chartSvgelements.textScale(sbb.width, this.w_width, 10);
    ...
    _createAxis() {
        ...
        // find the axes range
        this._lenY = chartSvgelements.axisRange(this.innerData, "y", intervalY, common.convertCSSBoolean(this._prop.yZero));
        if (this._label) {
            this._lenX = chartSvgelements.axisRange(this.innerData, "label", intervalX);
        }
        else {
            this._lenX = chartSvgelements.axisRange(this.innerData, "x", intervalX, common.convertCSSBoolean(this._prop.xZero));
        }
    ...
    _createLegend() {
        ...
        let dLegend = chartSvgelements.measureLegend(this.svg, this.w_width, this._legendNames, this._legendColors, this._legendThicknesses, optLg);
        ...
        if (!this._update) {
            ...
            chartSvgelements.createLegendLines(g, this._legendNames, this._legendColors, this._legendThicknesses, dLegend, this.w_width, rect.top, optLg);
        }
        else {
            g = this.svg.querySelector("#linechart-legend");
            chartSvgelements.removeChilds(g);
            chartSvgelements.createLegendLines(g, this._legendNames, this._legendColors, this._legendThicknesses, dLegend, this.w_width, rect.top, optLg);
        }
    }
    _createTitleY() {
    ...
    _createLine() {
        ...
        for (let l = 0; l < this.innerData.length; l++) {
            ...
            for (let i = 0; i < this.innerData[l].dataPoints.length; i++) {
                ...
                let pt = {};
                pt.y = chartSvgelements.axisConvertY(this._yaxis, this._lenY, this.innerData[l].dataPoints[i].y);
                if (this._lenX.interval && this._lenX.type === 'number') {
                    pt.x = chartSvgelements.axisConvertX(this._xaxis, this._lenX, this.innerData[l].dataPoints[i].x);
                }
                else {
            ...
            let length = chartSvgelements.getTotalLength(viewPt);
            if (length > 0) {
                opt.strokeLinejoin = 'round';
                opt.strokeLinecap = 'round';
                opt.strokeMiterlimit = '10';
                if (!this._update) {
                    plEl = chartSvgelements.createPolyline(g, points, opt);
                }
                else {
                    plEl = chartSvgelements.updatePolyline(this.svg, opt.id, points);
                }
                ...
                if (!this._update) {
                    plEl = chartSvgelements.createCircle(g, viewPt[0], 5, opt);
                }
                else {
                    plEl = chartSvgelements.updateCircle(this.svg, opt.id, viewPt[0], 5);
                }
            }
    ...
    _setAnimation(el, length, duration) {
        ...
        animOpt.fill = "freeze";
        chartSvgelements.createAnimation(el, animOpt);
    }
    _placeMarker(id, g, pt, line, index, scale, color) {
        let use;
        let symb;
        if (!this._update) {
            symb = chartSvgelements.createSVGElement('g', g);
            use = chartSvgelements.createSVGElement('use', symb);
        }
        else {
    ...
    _handleEventTarget(evt, pt) {
        ...
        this._removeLabel(this.svg);
        let nearestPoint = chartSvgelements.getNearest(this._Points, pt);
        ...
        chartSvgelements.createLineLabel(this.svg, label, nearestPoint, color, opt);
    }
    /* ---- Deal with rendering  */
    async _renderChart() {
