/* jqPlot 1.0.8r1250 | (c) 2009-2013 Chris Leonello | jplot.com
  ...
  function R() {
    ...
    this.noDataIndicator = {
      ...
      axes: { xaxis: { min: 0, max: 10, tickInterval: 2, show: true }, yaxis: { min: 0, max: 12, tickInterval: 3, show: true } },
    };
    ...
    this.parseOptions = function (ay) {
      ...
      for (var at = 0; at < this.data.length; at++) {
        var al = L.extend(
          true,
          { index: at },
          { seriesColors: this.seriesColors, negativeSeriesColors: this.negativeSeriesColors },
    ...
    this.draw = function () {
      if (this.drawIfHidden || this.target.is(':visible')) {
        ...
        ar.xaxis.pack(
          { position: 'absolute', bottom: this._gridPadding.bottom - ar.xaxis.getHeight(), left: 0, width: this._width },
          { min: this._gridPadding.left, max: this._width - this._gridPadding.right },
        );
        ar.yaxis.pack(
          { position: 'absolute', top: 0, left: this._gridPadding.left - ar.yaxis.getWidth(), height: this._height },
          { min: this._height - this._gridPadding.bottom, max: this._gridPadding.top },
        );
        ar.x2axis.pack(
          { position: 'absolute', top: this._gridPadding.top - ar.x2axis.getHeight(), left: 0, width: this._width },
          { min: this._gridPadding.left, max: this._width - this._gridPadding.right },
        );
        for (aH = 8; aH > 0; aH--) {
          ar[aG[aH - 1]].pack(
            { position: 'absolute', top: 0, right: this._gridPadding.right - az[aH - 1] },
            { min: this._height - this._gridPadding.bottom, max: this._gridPadding.top },
          );
        }
  ...
  L.extend(L.jqplot.effects, {
    ...
    createWrapper: function (ai) {
      ...
      var aj = { width: ai.outerWidth(true), height: ai.outerHeight(true), float: ai.css('float') },
        al = L('<div></div>').addClass('ui-effects-wrapper').css({ fontSize: '100%', background: 'transparent', border: 'none', margin: 0, padding: 0 }),
