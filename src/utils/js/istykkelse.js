...
function PlottFinnAkseVerdier(akse, ymin, ymax, nsugg) {
  ...
  if (suggint <= 2) suggint = 2 * faktor;
  else if (suggint <= 5) suggint = 5 * faktor;
  else suggint = 10 * faktor;
...
function PlottRektanglerIstykkelse(myObject, tobj, plott) {
  ...
  for (i = 0; i < n; i++) {
    ...
    if (plott.istype[j].patterndirection == 0) {
      ...
      trect.tooltipFormatString = SettInnLinjeskift(tobj.islag[i].istypenavn, 10, 2) + '<br> ' + (tobj.islag[i].y2 - tobj.islag[i].y1) + ' cm';
      ...
      tpattern = 1;
      while (lasty > tobj.islag[i].y1 + tint * 0.1) {
        // Trekker fra 10 % av intervallet for å unngå avrundingsfeil
        ...
        if (tpattern == 1) trect.color = plott.istype[j].fargeindre1;
        else trect.color = plott.istype[j].fargeindre2;
        trect.showTooltip = true;
        trect.tooltipLocation = 'n';
        trect.tooltipOffset = '10';
        trect.tooltipFormatString = SettInnLinjeskift(tobj.islag[i].istypenavn, 10, 2) + '<br> ' + (tobj.islag[i].y2 - tobj.islag[i].y1) + ' cm';
