...
export class UserSettingService {
  ...
  constructor() {
    //private translate: TranslateService, private loggingService: LoggingService) {
    //super();
    ...
    this.appModeLanguageAndCurrentGeoHazard$ = combineLatest([this.appMode$, this.language$, this.currentGeoHazard$]).pipe(shareReplay(1));

    this.daysBack$ = this.userSetting$.pipe(
      map(val => val.observationDaysBack),
      tap(val => console.log('days back changed to: ' + val)),
    ...
    (this.daysBackForCurrentGeoHazard$ = combineLatest([this.daysBack$, this.currentGeoHazard$]).pipe(
      ...
      tap(val => console.log('days back for current geohazard changed to: ' + val));
  ...
  private createSaveToDbOnChangeListener() {
    this.userSettingInMemory
      .pipe(
        ...
        debounceTime(200),
        tap(
          result => console.log('inmemory user settings changed. saving to db. result: ', result),
...
*/
  ...
  async saveGeoHazardsAndDaysBack({ geoHazards, daysBack }: { geoHazards?: number[] | undefined; daysBack?: number | null }): Promise<boolean | number> {
  ...
  async resetDaysBackForCurrentGeoHazard(): Promise<void> {
    ...
    const defaultDaysBackForCurrentGeoHazard = DEFAULT_USER_SETTINGS(null).observationDaysBack.find(x => x.geoHazard === currentGeoHazards[0]);
