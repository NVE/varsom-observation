...
export class VarsomStaticMap {
  ...
  getTileProperties(
    ...
    for (let tileY = cornerTileY; tileY * tileSize < y0 + h; tileY++) {
      for (let tileX = cornerTileX; tileX * tileSize < x0 + w; tileX++) {
        const url = this.mapLayerService.getUrlForTile(
          //mapId,
          config,
          tileX,
          tileY,
  ...
  getDamagePositions(obs: Observation) {
    if (obs && obs._damageObs && obs._damageObs.some(d => d.DamagePosition)) {
      return obs._damageObs
        .filter(d => d.DamagePosition && d.DamagePosition.Latitude && d.DamagePosition.Longitude)
  ...
  */
  ...
  createGraphics(positions: PositionToPlot[], polygons: PolygonsToPlot, { w: x0, n: y0, zoom }: MercatorBounds) {
    // Reset map graphics
    this.graphics = [];
    let start = null;
    let stop = null;
  ...
  createPolygons(polygons: LatLng[], w: number, n: number, zoom: number, fill: string) {
    ...
    const polylinesPointsToString = this.createPolylinesPointsFromMercatorPoints(mercatorPoints, svg_x0, svg_y0).flat().join(',');
  ...
  createCenterMarker(topPx: number, leftPx: number) {
    const location = this.getLocation(this.observation);
    const svg = RegobsGeoHazardMarker.getIconSvg(location.geoHazard); //this.sanitizer.bypassSecurityTrustHtml( //)
  ...
  createStartGraphic(topPx: number, leftPx: number) {
    ...
    this.startGraphic.push({
      id: 'start',
      svg: `<svg style="position:absolute; left: ${leftPx - w / 2}px; top: ${
        topPx - h
      }px" width="14" height="21" viewBox="0 0 14 21" fill="none" xmlns="http://www.w3.org/2000/svg">
 ...
 */
  ...
  render() {
    return (
      <div tabIndex={0} class={this.small ? 'container-small' : 'container'} ref={el => (this.container = el as HTMLElement)}>
        {this.tiles.map(el => {
          return (
            <div ref={tile => (this.tilesElement[el.count] = tile as HTMLElement)}>
              <img
                src={el.src}
                class="tile"
                //loading="lazy"
        ...
        {this.startGraphic.map(el => {
          return <div class="graphic-start-stop" innerHTML={el.svg} ref={graphic => (this.startGraphicElement = graphic as HTMLElement)}></div>;
        })}

        {this.stopGraphic.map(el => {
          return <div class="graphic-start-stop" innerHTML={el.svg} ref={graphic => (this.stopGraphicElement = graphic as HTMLElement)}></div>;
        })}

        {this.StartStopLineGraphic.map(el => {
          return <div class="graphic" innerHTML={el.svg} ref={graphic => (this.StartStopLineGraphicElement = graphic as HTMLElement)}></div>;
        })}
